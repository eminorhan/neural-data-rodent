...
#SBATCH --output=merge_ds_%A_%a.out
#SBATCH --array=0
#SBATCH --qos=debug
...
# run with torchrun
srun torchrun --nnodes $SLURM_NNODES --nproc_per_node 8 --max_restarts 1 --node_rank $SLURM_NODEID --rdzv_id 101 --rdzv_backend c10d --rdzv_endpoint "$MASTER_ADDR:$MASTER_PORT" ./merge_datasets.py
